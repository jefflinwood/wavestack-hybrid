...
- **Lane Diversity Regularizers:** Introduce configurable similarity/energy penalties plus per-lane logging to quantify specialization. (completed: cosine/energy configs + lane stat logging)
...
- **Parameter & FLOP Accounting:** Extend `ModelConfig.get_param_count()` and related tooling to report per-lane parameter counts and FLOP estimates so comparisons with attention baselines are grounded. (completed)
...
## Engineering Updates
- **Training Optimization Pass:** Add gradient accumulation and dataloader tuning (num_workers, pin_memory, prefetch_factor) while keeping defaults stable. (completed)
...
- **Lane Diversity Follow-up:** Run multi-seed baseline vs cosine diversity regularizer (optionally energy) to assess stability. (completed)
