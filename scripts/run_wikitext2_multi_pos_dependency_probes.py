#!/usr/bin/env python
"""Run POS/dependency probes across multiple representations."""
...
) -> Tuple[List[List[int]], List[str], List[str], List[str], List[str]]:
...
def _prepare_alignment_labels(
    payload: Dict[str, object], nlp
) -> Tuple[List[List[List[int]]], Dict[str, List[str]]]:
    tokenizer = TokenizerWrapper()
    input_ids = payload["input_ids"].tolist()
    mask = payload["mask"].tolist()

    alignments_per_sample: List[List[List[int]]] = []
    task_labels: Dict[str, List[str]] = defaultdict(list)

    for tokens, mask_row in zip(input_ids, mask):
        alignments, pos, dep, head_dir, head_bins = _align_spacy_to_bpe(
            tokenizer, tokens, mask_row, nlp
        )
        alignments_per_sample.append(alignments)
        if not alignments:
            continue
    ...
    return alignments_per_sample, task_labels


def _build_tasks(task_labels: Dict[str, List[str]]) -> Dict[str, ProbeTask]:
...
def _features_from_alignments(
    payload: Dict[str, object], alignments_per_sample: List[List[List[int]]]
) -> Dict[str, torch.Tensor]:
    reps = _collect_features(payload)
    rep_features: Dict[str, List[torch.Tensor]] = {name: [] for name in reps}

    for sample_idx, alignments in enumerate(alignments_per_sample):
        if not alignments:
            continue
    ...
    for rep_name, tensors in rep_features.items():
        if tensors:
            stacked[rep_name] = torch.cat(tensors, dim=0)
    return stacked
...
def _parse_inputs(values: List[str]) -> Dict[str, Path]:
    ...
    for entry in values:
        ...
        inputs[name.strip()] = Path(path.strip())
    return inputs
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="One or more name=path entries for probe payloads (pool=none).",
    )
    ...
    parser.add_argument(
        "--output",
        default="outputs/probes/wikitext2_phase4_pos_dep_results.jsonl",
    ...
    if not inputs:
        raise SystemExit("No probe payloads provided.")

    nlp = _load_spacy(args.spacy_model)
    first_payload = torch.load(next(iter(inputs.values())), map_location="cpu")
    alignments_per_sample, task_labels = _prepare_alignment_labels(first_payload, nlp)
    tasks = _build_tasks(task_labels)

    results: List[Dict[str, object]] = []
    for label, path in inputs.items():
        payload = torch.load(path, map_location="cpu")
        features = _features_from_alignments(payload, alignments_per_sample)
